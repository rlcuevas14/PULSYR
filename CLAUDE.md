...
- **i18n**: full UI in English (default) / Spanish: JSON catalogs + `t()`/`tn()`, language selector in navbar/login/setup (see Conventions).
...
| `i18n/` | JSON catalogs (`locales/{en,es}.json`), `t()`/`tn()` (fallback lang→en→key), `resolve_lang` (session, default `en`) |
...
- **i18n (UI copy)**: NEVER hardcode user-visible strings in templates or UI routers. Templates use the request-scoped Jinja globals `t("domain.key")` / `tn("domain.key", n)`; Python uses `app.i18n.t(key, resolve_lang(request))`. Catalogs: `app/i18n/locales/{en,es}.json` (flat dot-namespaced keys; English is source of truth and fallback). Enum display labels: `t("status." ~ x)` / `type.*` / `origin.*` / `stage.*`. Language selector lives in the navbar (session-based, `GET /ui/lang/{code}`); default English. `tests/test_i18n.py` enforces catalog completeness, placeholder parity, and template coverage **for exactly the languages in `LANGS`**: adding a key to one catalog without the other fails CI. A catalog file that is not in `LANGS` is never loaded and never checked, which is how a French one drifted 28 keys behind before being removed in `v2026.08.24`. Careful: a Jinja loop variable named `t` shadows the translation global: never use it.
